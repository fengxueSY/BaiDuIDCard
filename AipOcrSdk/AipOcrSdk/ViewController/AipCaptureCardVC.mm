...
//#error 改动本类的代码逻辑时注意！！！ AipCaptureCardVC类使用了GCD用做扫描帧时的处理，页面释放前一定要保证GCD信号量的正常释放，相关代码参考478行相关的处理逻辑。1.dispatch_semaphore_signal(_sema);2.self.recognizedSucceed = NO;
...
- (void)reset{
    ...
    [self.checkChooseBtn setTitle:@"识别" forState:UIControlStateNormal];
}
...
- (IBAction)pressCheckChoose:(id)sender {
    ...
    if ([self.checkChooseBtn.currentTitle isEqualToString:@"完成"]) {
    ...
    switch (self.cardType) {
        case CardTypeIdCardFont:{
            [[AipOcrService shardService] detectIdCardFrontFromImage:self.finalImage withOptions:nil successHandler:^(id result) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self showIDCardMessage:@"识别成功"];
                    [self changecheckChooseButtonTitle];
                });
                if ([self.delegate respondsToSelector:@selector(ocrOnIdCardSuccessful: AndImage:)]) {
                    [self.delegate ocrOnIdCardSuccessful:result  AndImage:self.idCardImage];
                    //这里添加返回，使不用点击完成就可以返回上个界面
...
-(void)changecheckChooseButtonTitle{
    if ([self.checkChooseBtn.currentTitle isEqualToString:@"识别"]) {
        [self.checkChooseBtn setTitle:@"完成" forState:UIControlStateNormal];
    }else{
        [self.checkChooseBtn setTitle:@"识别" forState:UIControlStateNormal];
    }
}
-(void)showIDCardMessage:(NSString *)message{
    self.tipsLabel.text = MyLocal(message);
}
//重新拍照
...
+(UIViewController *)ViewControllerWithCardType:(CardType)type andDelegate:(id<AipOcrDelegate>)delegate isPushTypeOrAlbum:(UIImage *)isAlbumImage{
    ...
    if (isAlbumImage != nil) {
        vc.isOpenPhotoAlbum = isAlbumImage;
    }
    AipNavigationController *navController = [[AipNavigationController alloc] initWithRootViewController:vc];
    return navController;
}
#pragma mark - 拿到传进来的照片后，开始解析照片
-(void)analysicUserIDCardPhoto{
    [self setupCutImageView:self.isOpenPhotoAlbum fromPhotoLib:YES];
    //对于相册选取的照片暂时切换状态
    if (self.cardType == CardTypeLocalIdCardFont) {
        ...
        case idcard_quality::IDCARD_NORMAL:{
            ...
            if (isBack) {
                ...
                [[AipOcrService shardService] detectIdCardBackFromImage:self.finalImage withOptions:nil successHandler:^(id result) {
                    ...
                    [wself ocrOnIdCardSuccessful:result AndImage:wself.idCardImage];
                } failHandler:^(NSError *err) {
